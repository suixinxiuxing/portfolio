  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/90 backdrop-blur-xl shadow-sm border-b border-gray-100 py-3" : "bg-transparent py-5"}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <motion.a href="#hero" className="text-base font-bold tracking-tight text-gray-900" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          Xi<span className="text-[#8b5cf6]"> Chen</span>
        </motion.a>
        <div className="hidden md:flex items-center gap-1">
          {links.map((l, i) => (
            <motion.a key={l.href} href={l.href} className="px-3 py-2 text-[10px]font-medium text-gray-500 hover:text-[#8b5cf6] transition-colors rounded-lg hover:bg-gray-50 uppercase tracking-wider" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>{l.label}</motion.a>
          ))}
          <button onClick={toggleLang} className="ml-2 px-3 py-2 text-[10px]font-medium text-gray-400 hover:text-[#8b5cf6] transition-colors rounded-lg hover:bg-gray-50 uppercase tracking-wider border border-gray-200">{lang === "zh" ? "EN" : "中"}</button>
        </div>
      ...
      <AnimatePresence>{open && (
        <motion.div className="md:hidden bg-white border-b border-gray-100" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
          <div className="px-6 py-4 flex flex-col gap-1">{links.map(l => <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="py-2 text-[10px]text-gray-500 hover:text-[#8b5cf6] transition-colors uppercase tracking-wider">{l.label}</a>)}</div>
        </motion.div>
      )}</AnimatePresence>
    </nav>
  );
