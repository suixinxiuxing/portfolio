    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/[0.04] py-3" : "bg-transparent py-5"}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          {/* Language toggle */}
          <button onClick={toggleLang} className="ml-2 px-3 py-2 text-xs font-medium text-white/40 hover:text-[#a78bfa] transition-colors duration-200 rounded-lg hover:bg-white/[0.04] uppercase tracking-wider border border-white/[0.06]">
            {lang === "zh" ? "EN" : "中"}
          </button>
        </div>

        <div className="md:hidden flex items-center gap-2">
          <button onClick={toggleLang} className="px-2 py-1 text-[10px] font-medium text-white/40 hover:text-[#a78bfa] transition-colors rounded border border-white/[0.06]">
            {lang === "zh" ? "EN" : "中"}
          </button>
          <button className="text-white/50 hover:text-white" onClick={() => setOpen(!open)}>{open ? <HiX size={22} /> : <HiMenuAlt3 size={22} />}</button>
        </div>
      </div>
