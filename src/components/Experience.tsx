      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-5xl mx-auto mb-20">
          <motion.div className="flex items-center gap-3 mb-8" initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="w-8 h-8 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-center text-[#7c3aed] text-sm font-bold">01</div>
            <div><span className="tag-framer text-xs">{tt("experience.internship")}</span><h3 className="text-2xl font-bold text-gray-900 mt-1">{tt("experience.internshipTitle")}</h3></div>
          </motion.div>
          {internship.map((e, i) => (
            <motion.div key={e.title} className="card-framer p-7 sm:p-8" variants={item} initial="hidden" whileInView="visible" viewport={{ once: true }}>
              <h3 className="text-xl font-bold text-gray-900 mb-1">{e.title}</h3>
        ...
        <div className="max-w-5xl mx-auto">
          <motion.div className="flex items-center gap-3 mb-8" initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-[#3b82f6] text-sm font-bold">02</div>
            <div><span className="tag-framer tag-framer-muted text-xs">{tt("experience.campus")}</span><h3 className="text-2xl font-bold text-gray-900 mt-1">{tt("experience.campusTitle")}</h3></div>
          </motion.div>
          <motion.div className="grid md:grid-cols-2 gap-4" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
            {campus.map((e, i) => (
              <motion.div key={e.title} className="card-framer p-7 flex flex-col" variants={item}>
                ...
                <p className="text-sm text-gray-400 mb-3">{e.org}</p>
                <div className="flex gap-3 text-xs text-gray-400 mb-4"><span className="flex items-center gap-1"><FiClock size={10}/>{e.period}</span><span className="flex items-center gap-1"><FiMapPin size={10}/>{e.loc}</span></div>
